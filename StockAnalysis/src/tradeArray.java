...
public class tradeArray {
    ...
    public Stats getStats() {
        ...
        if(st.numberOfTrades == 0){
            st.percentOfWinners = st.avgProfit = st.avgHoldingPeriod = 0;
        } else{
            st.percentOfWinners = (double) st.numberWinners / (double)st.numberOfTrades * 100;
            st.avgProfit = (st.totalWinnings + st.totalLoss) / (double)st.numberOfTrades;
            st.avgHoldingPeriod = (double)st.numberDays / (double)st.numberOfTrades;
        }

        if(st.numberLong == 0){
            st.percentOfLongWinners = st.avgLongProf = 0;
        } else {
            st.percentOfLongWinners = (double)st.longWinners / (double)st.numberLong * 100;
            st.avgLongProf = (st.totalLongWinnings + st.totalLongLoss) / (double)st.numberLong;
        }

        if(st.numberShort == 0){
            st.percentOfShortWinners = st.avgShortProf = 0;
        } else {
            st.percentOfShortWinners = (double)st.shortWinners / (double)st.numberShort * 100;
            st.avgShortProf = (st.totalShortWinnings + st.totalShortLoss) / (double)st.numberShort;
        }
